...
serializeCardData = async pages => {
  const respDom = await JSDOM.fromURL(
    `https://www.roguard.net/db/cards/${pages}`,
  );
  ...
  for (const element of match) {
    ...
    permaBuff.push(normalizeText(splitEffectCardDiv[3].match(/<div>(.*)/)[1]));

    const cardSlot =
      splitSlotCardDiv[1].match(/<div>(.*)/)[1] === '?'
        ? null
        : splitSlotCardDiv[1].match(/<div>(.*)/)[1];
    const cardType =
      splitSlotCardDiv[3].match(/<div>(.*)/)[1] === '?'
        ? null
        : splitSlotCardDiv[3].match(/<div>(.*)/)[1];
    ...
    if (splitEffectCardDiv[1].match(/<br ?\/?>/)) {
      ...
      effectCardDiv.forEach((element, index) => {
        if (index === 0) {
          cardEffect.push(normalizeText(element.match(/<div>(.*)/)[1]));
        } else {
          cardEffect.push(normalizeText(element.match(/\r\n|\r|\n(.*)/)[1]));
        }
      });
      // if effect card has single lines
    } else {
      cardEffect.push(normalizeText(splitEffectCardDiv[1].match(/<div>(.*)/)[1]));
    }
...
serializeCardFullData = async url => {
  ...
  // common data
  const cardLevel = commonMatch[1].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const cardMaxStack = commonMatch[2].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const cardSellable = commonMatch[3].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const cardSellPrice = commonMatch[4].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const cardAuctionable = commonMatch[5].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const cardStorageable =
    commonMatch[6] && commonMatch[6].match(/<td class="text-right">(.*)<\/td>/)
      ? commonMatch[6].match(/<td class="text-right">(.*)<\/td>/)[1]
      : null;
  ...
  if (splitDiv[3]) {
    const dropDom = splitDiv[3].split(
    ...
    // drop data
    const cardDroppedBy =
      dropElement[0] && dropElement[0].match(/<a .*>(.*)<\/a>/)
        ? dropElement[0].match(/<a .*>(.*)<\/a>/)[1]
        : null;
    const cardMonsterLevel =
      dropElement[1] && dropElement[1].match(/.*>(.*)/)
        ? dropElement[1].match(/.*>(.*)/)[1]
        : null;
    const cardDropRate =
      dropElement[2] && dropElement[2].match(/.*>(.*)/)
        ? dropElement[2].match(/.*>(.*)/)[1]
        : null;
