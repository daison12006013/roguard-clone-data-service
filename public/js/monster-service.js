...
serializeMonsterData = async pages => {
  const respDom = await JSDOM.fromURL(
    `https://www.roguard.net/db/monsters/${pages}`,
  );
  ...
  for (const element of match) {
    ...
    const monsterElement = element.match(
      /<div style="color: (.*);">(.+?) • (.*)<\/div>/,
    )[2];
    const monsterRace = element.match(
      /<div style="color: (.*);">(.+?) • (.*)<\/div>/,
    )[3];
    const monsterInfo = element.match(
    ...
    const monsterExp = element.match(
...
serializeMonsterFullData = async url => {
  ...
  const splitDiv = content.split(
    '<div style="width: 230px; display: inline-block; margin-top: 10px; vertical-align:top; margin-right: 5px;">',
  );

  // monster description
  const monsterDescription = splitDiv[0].match(
    /<p style="padding: 10px;">(.*)<\/p>/,
  )[1];

  // common data dom element
  const commonDom = new JSDOM(splitDiv[1]);
  const commonTbody = commonDom.window.document.querySelector('tbody')
    .innerHTML;
  const commonMatch = commonTbody.match(/<tr[\s\S]*?<\/tr>/g);

  // common data
  const monsterLevel = commonMatch[1].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterType = commonMatch[2].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterZone = commonMatch[3].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterRace = commonMatch[4].match(
    /<td class="text-right"><a href=".*">(.*)<\/a><\/td>/,
  )[1];
  const monsterElement = commonMatch[5].match(
    /<td class="text-right"><a href=".*">(.*)<\/a><\/td>/,
  )[1];
  const monsterSize = commonMatch[6].match(
    /<td class="text-right"><a href=".*">(.*)<\/a><\/td>/,
  )[1];
  const monsterPassiveLevel = commonMatch[7].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterBaseExp = commonMatch[8].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterJobExp = commonMatch[9].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];

  // attributes data dom element
  const attributesDom = new JSDOM(splitDiv[2]);
  const attributesTbody = attributesDom.window.document.querySelector('tbody')
    .innerHTML;
  const attributesMatch = attributesTbody.match(/<tr[\s\S]*?<\/tr>/g);

  // attributes data
  const monsterATK = attributesMatch[0].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterMATK = attributesMatch[1].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterDEF = attributesMatch[2].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterMDEF = attributesMatch[3].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterHP = attributesMatch[4].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterHit = attributesMatch[5].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterFlee = attributesMatch[6].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterMoveSpd = attributesMatch[7].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];
  const monsterATKSpd = attributesMatch[8].match(
    /<td class="text-right">(.*)<\/td>/,
  )[1];

  // card data dom element
  const cardTbody = attributesDom.window.document.querySelector('div')
    ? attributesDom.window.document.querySelector('div').innerHTML
    : null;
  ...
  const cardImg =
    cardMatch &&
    cardMatch[0].match(/src="\/\/(.*)"/) &&
    cardMatch[0].match(/src="\/\/(.*)"/)[1]
      ? cardMatch[0].match(/src="\/\/(.*)"/)[1]
      : null;
  const cardName =
    cardMatch &&
    cardMatch[1] &&
    cardMatch[1].match(/<a .*>(.*)<\/a>/) &&
    cardMatch[1].match(/<a .*>(.*)<\/a>/)[1]
      ? cardMatch[1].match(/<a .*>(.*)<\/a>/)[1]
      : null;
  ...
  if (
    cardMatch &&
    cardMatch[3] &&
    cardMatch[3].match(/<div .*>(.*)<br>\n(.*)<\/div>/)
  ) {
    cardEffect.push(normalizeText(cardMatch[3]
      .match(/<div .*>((.*)<br>\n(.*))<\/div>/)[1]
      .replace('<br>\n', ', ')));
  } else {
    cardEffect.push(normalizeText(
      cardMatch &&
      cardMatch[3] &&
      cardMatch[3].match(/<div .*>(.*)<\/div>/) &&
      cardMatch[3].match(/<div .*>(.*)<\/div>/)[1]
        ? cardMatch[3].match(/<div .*>(.*)<\/div>/)[1]
        : null));
  }
  ...
  if (
    cardMatch &&
    cardMatch[5] &&
    cardMatch[5].match(/<div .*>(.*)<br>\n(.*)<\/div>/)
  ) {
    cardBuff.push(normalizeText(cardMatch[5]
      .match(/<div .*>((.*)<br>\n(.*))<\/div>/)[1]
      .replace('<br>\n', ', ')));
  } else {
    cardBuff.push(normalizeText(
      cardMatch && cardMatch[5] && cardMatch[5].match(/<div .*>(.*)<\/div>/)
        ? cardMatch[5].match(/<div .*>(.*)<\/div>/)[1]
        : null));
  }

  // drop data dom element
  const dropDom = splitDiv[2].split(
    '<div style="width: 230px; display: inline-block; margin-top: 10px; vertical-align:top;">',
  );
  const dropElement = dropDom[2]
    ? dropDom[2]
        .split('<div style="text-align: center; margin-bottom: 4px;">')
        .slice(1)
    : [];
  ...
  const locationElement = dropDom[3]
    ? dropDom[3]
        .split('<div style="text-align: center; margin-top: 5px;">')
        .slice(1)
    : [];
